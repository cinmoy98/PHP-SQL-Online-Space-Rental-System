...
INNER JOIN userlo ON training.uid=userlo.uid
");
 ...
 if($_POST['fspace'] != "")
   ...
   if($fmethod== "" && $farea == "")
   {
     if($srtsz == "LH" && $srtprc == "LH"){
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
       WHERE training.size<='$fsize' ORDER BY training.price ASC,training.size ASC ");
      }
     else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
       WHERE training.size<='$fsize' ORDER BY training.price DESC, training.size ASC ");
      }
      else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
       WHERE training.size<='$fsize' ORDER BY training.price DESC,training.size DESC");
      }
      else
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
       WHERE training.size<='$fsize' ORDER BY training.price ASC,training.size DESC");
   ...
   else if($fmethod== "")
   {
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM training
     INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.area='$farea' ORDER BY training.price ASC,training.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
     INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.area='$farea' ORDER BY training.price DESC, training.size ASC");
     }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
     INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.area='$farea' ORDER BY training.price DESC,training.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM training
     INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.area='$farea' ORDER BY  training.price ASC,training.size DESC");
   }
   ...
   else if($farea == "")
   {
    ...
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.pricing='$fmethod'  ORDER BY training.price ASC,training.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.pricing='$fmethod'  training.price DESC, training.size ASC");
      }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.pricing='$fmethod'  ORDER BY training.price DESC,training.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM training
       INNER JOIN userlo ON training.uid=userlo.uid
      WHERE training.size<='$fsize' AND training.pricing='$fmethod'  ORDER BY  training.price ASC,training.size DESC");
