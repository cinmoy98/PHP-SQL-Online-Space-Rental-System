...
INNER JOIN userlo ON conference.uid=userlo.uid
");
 ...
 if($_POST['fspace'] != "")
   ...
   if($fmethod== "" && $farea == "")
   {
     if($srtsz == "LH" && $srtprc == "LH"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
       WHERE conference.size<='$fsize' ORDER BY conference.price ASC,conference.size ASC ");
      }
     else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
       WHERE conference.size<='$fsize' ORDER BY conference.price DESC, conference.size ASC ");
      }
      else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
       WHERE conference.size<='$fsize' ORDER BY conference.price DESC,conference.size DESC");
      }
      else
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
       WHERE conference.size<='$fsize' ORDER BY conference.price ASC,conference.size DESC");
   ...
   else if($fmethod== "")
   {
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM conference
     INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.area='$farea' ORDER BY conference.price ASC,conference.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
     INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.area='$farea' ORDER BY conference.price DESC, conference.size ASC");
     }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
     INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.area='$farea' ORDER BY conference.price DESC,conference.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM conference
     INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.area='$farea' ORDER BY  conference.price ASC,conference.size DESC");
   }
   ...
   else if($farea == "")
   {
    ...
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.pricing='$fmethod'  ORDER BY conference.price ASC,conference.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.pricing='$fmethod'  conference.price DESC, conference.size ASC");
      }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.pricing='$fmethod'  ORDER BY conference.price DESC,conference.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM conference
       INNER JOIN userlo ON conference.uid=userlo.uid
      WHERE conference.size<='$fsize' AND conference.pricing='$fmethod'  ORDER BY  conference.price ASC,conference.size DESC");
