...
INNER JOIN userlo ON interview.uid=userlo.uid
");
 ...
 if($_POST['fspace'] != "")
   ...
   if($fmethod== "" && $farea == "")
   {
     if($srtsz == "LH" && $srtprc == "LH"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
       WHERE interview.size<='$fsize' ORDER BY interview.price ASC,interview.size ASC ");
      }
     else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
       WHERE interview.size<='$fsize' ORDER BY interview.price DESC, interview.size ASC ");
      }
      else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
       WHERE interview.size<='$fsize' ORDER BY interview.price DESC,interview.size DESC");
      }
      else
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
       WHERE interview.size<='$fsize' ORDER BY interview.price ASC,interview.size DESC");
   ...
   else if($fmethod== "")
   {
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM interview
     INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.area='$farea' ORDER BY interview.price ASC,interview.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
     INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.area='$farea' ORDER BY interview.price DESC, interview.size ASC");
     }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
     INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.area='$farea' ORDER BY interview.price DESC,interview.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM interview
     INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.area='$farea' ORDER BY  interview.price ASC,interview.size DESC");
   }
   ...
   else if($farea == "")
   {
    ...
    if($srtsz == "LH" && $srtprc == "LH"){
      ...
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.pricing='$fmethod'  ORDER BY interview.price ASC,interview.size ASC ");
      }

       else if($srtsz == "LH" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.pricing='$fmethod'  interview.price DESC, interview.size ASC");
      }

   else if($srtsz == "HL" && $srtprc == "HL"){
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.pricing='$fmethod'  ORDER BY interview.price DESC,interview.size DESC");
      }

      else
      $sql=mysqli_query($conn,"SELECT * FROM interview
       INNER JOIN userlo ON interview.uid=userlo.uid
      WHERE interview.size<='$fsize' AND interview.pricing='$fmethod'  ORDER BY  interview.price ASC,interview.size DESC");
